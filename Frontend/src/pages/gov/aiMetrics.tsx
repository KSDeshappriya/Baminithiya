...
const StatCard = ({ icon: Icon, label, value, color = 'indigo' }: {
  ...
  const colorClasses = {
    indigo: 'bg-white text-indigo-700 border border-indigo-100',
    green: 'bg-white text-green-700 border border-green-100',
    blue: 'bg-white text-blue-700 border border-blue-100',
    orange: 'bg-white text-orange-700 border border-orange-100'
  };
  ...
  return (
    <div className={`p-5 rounded-xl shadow-sm hover:shadow-md transition-shadow duration-200 ${colorClasses[color as keyof typeof colorClasses]} flex items-center gap-4`}> 
      <div className="flex items-center justify-center w-12 h-12 rounded-full bg-gray-50 border border-gray-100">
...
export const AIMetricsPage: React.FC = () => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="bg-white p-6 rounded-xl shadow-lg border border-red-200 max-w-md w-full text-center">
          ...
          <div className="text-lg font-bold text-gray-900 mb-2">AI Metrics Not Found</div>
          <div className="text-gray-600 mb-4">No data available for this disaster.</div>
          <button
            onClick={() => navigate(-1)}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white py-10 px-4 sm:px-8 lg:px-16 font-sans">
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="mb-8 border-b border-gray-200 pb-6 flex flex-col gap-2">
          <Link 
            to={`/gov/disaster/${disasterId}`} 
            className="inline-flex items-center text-indigo-600 hover:text-indigo-800 font-medium transition-colors text-sm group"
          >
            <ArrowLeftIcon className="w-4 h-4 mr-2 group-hover:-translate-x-1 transition-transform" /> Back to Disaster
          </Link>
          <div className="flex items-center justify-between mt-2">
            <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">AI Workflow <span className="text-indigo-500">#1</span></h1>
          </div>
        </div>
